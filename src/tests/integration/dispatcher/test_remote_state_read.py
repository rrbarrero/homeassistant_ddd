...
def test_dispatcher():
    ...
    assert aggregation_state_at(spy=spy, aggregate_repo=aggregate_repo, index=0).status == CurrentStateStatus.REMOTE_READ
    ...
    assert aggregation_state_at(spy=spy, aggregate_repo=aggregate_repo, index=1).status == CurrentStateStatus.MANAGED_DEVICE

    # 3rd exceedance is updated with the new consumption
    assert spy[2].current_state.exceedance == 1300
    assert aggregation_state_at(spy=spy, aggregate_repo=aggregate_repo, index=2).status == CurrentStateStatus.STATE_RECALCULATED
    ...
    assert aggregation_state_at(spy=spy, aggregate_repo=aggregate_repo, index=3).status == CurrentStateStatus.MANAGED_DEVICE

    # 5th exceedance is updated with the new consumption
    assert spy[4].current_state.exceedance == 700
    assert aggregation_state_at(spy=spy, aggregate_repo=aggregate_repo, index=4).status == CurrentStateStatus.STATE_RECALCULATED

    # No more room for new devices because the marginal offset (hardcoded to 400)
    assert spy[5].device_changed is None
    assert aggregation_state_at(spy=spy, aggregate_repo=aggregate_repo, index=5).status == CurrentStateStatus.MANAGED_DEVICE
