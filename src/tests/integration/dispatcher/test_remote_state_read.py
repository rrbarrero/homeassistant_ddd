...
def test_dispatcher():
    ...
    assert check_repo_state_at(index=0).status == CurrentStateStatus.REMOTE_READ
    ...
    assert check_repo_state_at(index=1).status == CurrentStateStatus.MANAGED_DEVICE

    # 3rd exceedance is updated with the new consumption
    assert spy[2].current_state.exceedance == 1300
    assert check_repo_state_at(index=2).status == CurrentStateStatus.STATE_RECALCULATED
    ...
    assert check_repo_state_at(index=3).status == CurrentStateStatus.MANAGED_DEVICE

    # 5th exceedance is updated with the new consumption
    assert spy[4].current_state.exceedance == 700
    assert check_repo_state_at(index=4).status == CurrentStateStatus.STATE_RECALCULATED

    # No more room for new devices because the marginal offset (hardcoded to 400)
    assert spy[5].device_changed is None
    assert check_repo_state_at(index=5).status == CurrentStateStatus.MANAGED_DEVICE
